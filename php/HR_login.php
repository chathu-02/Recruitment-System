...
if ($_SERVER["REQUEST_METHOD"] == "POST") {
    ...
    // Check the credentials (In a real application, validate against the database)
    if ($username == "HR" && $password == "hr123") {
        $_SESSION['logged_user'] = $username; // Store the logged-in user in the session
        
        // Redirect to hr_page.php if credentials are correct
        echo "<script>alert('Login successful'); window.location.href='hr_page.php';</script>";
    } else {
        // Display an alert and stay on the login page if credentials are incorrect
        echo "<script>alert('Incorrect username or password. Please try again.'); window.location.href='HR_login.php';</script>";
    }
}
?>
<!DOCTYPE Html>
...
<body>
    ...
    <div class="wrapper">
        <div class="form-box login">
            <h2>Login</h2>
            <!-- Note: The form action is posting to the same page for login validation -->
            <form method="POST" action="HR_login.php">
                ...
                <div class="login-register">
                    <p>Don't have an account? <a href="#" class="register-link">Register</a><br><br></p>
                </div>
