...
if ($_SERVER["REQUEST_METHOD"] == "POST") {
    ...
    // Replace the following with your actual validation logic (e.g., checking against a database)
    if ($username == "Rec" && $password == "123") {
        $_SESSION['logged_user'] = $username; // Store the logged-in user in the session
        echo "<script>
                alert('Login successful!');
                window.location.href = 'recruiterpage.php'; // Redirect to the recruiter page
              </script>";
        exit(); // Exit to prevent further execution
    } else {
        echo "<script>alert('You have entered the incorrect username or password.');</script>"; // Alert for incorrect login
    }
}
?>
<!DOCTYPE Html>
...
<body>
    ...
    <div class="wrapper">
        <div class="form-box login">
            <h2>Login</h2>
            <form method="POST" action=""> <!-- Submit to the same page -->
                <div class="input-box">
