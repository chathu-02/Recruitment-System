...
if ($_SERVER["REQUEST_METHOD"] == "POST") {
    ...
    if (empty($errors)) {
        ...
        $Eposition = mysqli_real_escape_string($conn, $Eposition);

        $sql = "UPDATE `employee` SET Ename='$Ename', Eaddress='$Eaddress', salary='$salary', Eposition='$Eposition' WHERE eid='$eid'";

        if (mysqli_query($conn, $sql)) {
            echo "<script>alert('Record updated successfully'); window.location.href='system.php';</script>";
            exit;
        } else {
...
<?php if (!empty($errors)) : ?>
    ...
    <h1>Edit Employee</h1>
        <div class="d1">
            ...
            <input type="text" id="eid" name="eid" value="<?php echo $eid; ?>" readonly>
        </div>
        <div class="d1">
